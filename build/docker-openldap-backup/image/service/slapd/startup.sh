...
cronfile="/container/service/slapd-backup/assets/cronjobs"
printf "# Add defailt nodes\n" >> $cronfile
printf "%s %s %s %s * root /container/service/add_nodes.sh" $[$(date +%M) + 2] $(date +%H) $(date +%d) $(date +%m) >> $cronfile
printf "\n# empty line" >> $cronfile
